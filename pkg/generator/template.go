...
// {{.Name}}Loadable defines the needed methods to interact with this entity.
type {{.Name}}Loadable interface {
	Load(ctx context.Context, key {{.KeyType.String}}) ({{.ValType.String}}, error)
	LoadAll(ctx context.Context, keys []{{.KeyType}}) ([]{{.ValType.String}}, []error)
	LoadThunk(ctx context.Context, key {{.KeyType.String}}) func() ({{.ValType.String}}, error)
	LoadAllThunk(ctx context.Context, keys []{{.KeyType}}) (func() ([]{{.ValType.String}}, []error))
	Prime(key {{.KeyType}}, value {{.ValType.String}}) bool
	Clear(key {{.KeyType}})
}
...
func New{{.Name}}(config {{.Name}}Config) {{.Name}}Loadable {
	return &{{.Name}}{
		fetch: config.Fetch,
		wait: config.Wait,
